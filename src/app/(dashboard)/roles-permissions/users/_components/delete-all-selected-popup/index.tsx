...
import { crud_delete_many_users } from "@/lib/curd/user";
...
export default function DeleteAllSelectedUsersPopup({
  selectedIds,
  unSelectedAll,
}: Props) {
  // rooms store
  const { remove_many_users } = useUsersStore();
  ...
  const handleDelete = async () => {
    ...
    try {
      // delete many users
      const res = await crud_delete_many_users(selectedIds);
      // remove the roles from the store
      if (res) {
        remove_many_users(selectedIds);
      }
      ...
      setOpen(false);
      // adding a toast
      toast.success("All Selected Users Where Deleted Successfully");
    } catch (err) {
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            <DialogTitle className="mb-2">
              Delete All {selectedIds.length} Selected Users
            </DialogTitle>
            <DialogDescription className="text-card-foreground">
              Please be shure before you delete these users because this action
              can not be undone
            </DialogDescription>
          </DialogHeader>
