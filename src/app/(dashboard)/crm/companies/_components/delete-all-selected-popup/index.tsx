...
import { crud_delete_many_crm_companies } from "@/lib/curd/crm-company";
...
export default function DeleteAllSelectedCRMCompaniesPopup({
  selectedIds,
  unSelectedAll,
}: Props) {
  // crm companies store hook
  const { remove_many_companies } = useCRMCompaniesStore();
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const res = await crud_delete_many_crm_companies(selectedIds);
      // remove the companies from the store
      if (res) {
        remove_many_companies(selectedIds);
      }
      ...
      setOpen(false);
      // adding a toast
      toast.success("All Selected Companies Where Deleted Successfully");
    } catch (err) {
        ...
        <div className="w-full h-full flex flex-col gap-4 justify-between">
          <DialogHeader>
            <DialogTitle className="mb-2">
              Delete All {selectedIds.length} Selected Companies
            </DialogTitle>
            <DialogDescription className="text-card-foreground">
              Please be shure before you delete these companies because this
