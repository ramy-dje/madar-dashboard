...
// The top crm companies industries Analytics Chart Card

interface Props {
  crm_companies_analytics_data: CRMCompanyAnalyticsInterface;
}

export default function TopCRMCompaniesIndustriesAnalyticsChartCard({
  crm_companies_analytics_data,
}: Props) {
  // Transform data for the chart
  const chartData = useMemo(
    () =>
      crm_companies_analytics_data.companies_top_industries
        .map((industry) => ({
      ...
      <CardHeader className="flex flex-row justify-between items-start p-0 mb-3 space-y-0">
        <div className="space-y-0.5">
          <CardTitle className="w-full text-lg">
            Top {chartData.length} CRM Companies Industries
          </CardTitle>
          <CardDescription className="w-full text-xs font-medium text-foreground/60">
            Overview of the top CRM companies industries
          </CardDescription>
        </div>
