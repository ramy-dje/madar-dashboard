...
// The top crm companies regions Analytics Chart Card

interface Props {
  crm_companies_analytics_data: CRMCompanyAnalyticsInterface;
}

export default function TopCRMCompaniesRegionsAnalyticsChartCard({
  crm_companies_analytics_data,
}: Props) {
  // Transform data for the chart
  const chartData = useMemo(
    () =>
      crm_companies_analytics_data.companies_top_regions
        .map((country) => ({
          name: country.region,
      ...
      <CardHeader className="flex flex-row justify-between items-start p-0 mb-3 space-y-0">
        <div className="space-y-0.5">
          <CardTitle className="w-full text-lg">
            Top {chartData.length} CRM Companies Regions
          </CardTitle>
          <CardDescription className="w-full text-xs font-medium text-foreground/60">
            Overview of the top CRM companies regions
          </CardDescription>
        </div>
        <div className="flex flex-col items-end">
          ...
          <p className="text-foreground/60 text-xs">Region(s)</p>
        </div>
      </CardHeader>
      ...
      <CardContent className="w-full flex-1 p-0">
        <ChartContainer config={chartConfig} className="max-h-[250px]">
          ...
          >
            ...
            />
            <Bar dataKey="total" layout="vertical" fill="#FF4500" radius={4}>
              <LabelList
                dataKey="name"
                position="insideLeft"
                offset={10}
