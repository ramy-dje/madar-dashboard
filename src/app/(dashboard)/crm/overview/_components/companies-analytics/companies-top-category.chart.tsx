...
// The top crm companies categories Analytics Chart Card

interface Props {
  crm_companies_analytics_data: CRMCompanyAnalyticsInterface;
}

export default function TopCRMCompaniesCategoriesAnalyticsChartCard({
  crm_companies_analytics_data,
}: Props) {
  // Transform data for the chart
  const chartData = useMemo(
    () =>
      crm_companies_analytics_data.companies_top_categories
        .map((category) => ({
          name: category.category,
          total: category.companies,
        }))
      ...
      <CardHeader className="flex flex-row justify-between items-start p-0 mb-3 space-y-0">
        <div className="space-y-0.5">
          <CardTitle className="w-full text-lg">
            Top {chartData.length} CRM Companies Categories
          </CardTitle>
          <CardDescription className="w-full text-xs font-medium text-foreground/60">
            Overview of the top CRM companies categories
          </CardDescription>
        </div>
        <div className="flex flex-col items-end">
          ...
          <p className="text-foreground/60 text-xs">Category(s)</p>
        </div>
      </CardHeader>
