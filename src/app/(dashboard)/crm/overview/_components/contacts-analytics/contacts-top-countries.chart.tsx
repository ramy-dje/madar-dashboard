...
// The top crm contacts countries Analytics Chart Card

interface Props {
  crm_contacts_analytics_data: CRMContactAnalyticsInterface;
}

export default function TopCRMContactsCountriesAnalyticsChartCard({
  crm_contacts_analytics_data,
}: Props) {
  // Transform data for the chart
  const chartData = useMemo(
    () =>
      crm_contacts_analytics_data.contacts_top_countries
        .map((country) => ({
          name: country.country,
          total: country.contacts,
        }))
  ...
  // total
  const total_reservations = useMemo(() => {
