...
export const DropZoneCard = ({ placeholder }: { placeholder: string }) => (
  <div className="flex items-center gap-3">
    {/* <HiCloudUpload className="size-10 text-accent-foreground/80" /> */}
    <i className="size-12 text-accent-foreground/80">
...
const getErrorMessage = (
  ...
  const errorGroups: Record<string, { files: string[]; message: string }> = {};
  ...
  fileRejections.forEach((rejection) => {
    rejection.errors.forEach((error) => {
      ...
      errorGroups[error.code].files.push(rejection.file.name);
    });
  });

  // Format custom messages based on error types
  const messages = [];

  for (const code in errorGroups) {
    const { message, files } = errorGroups[code];

    // Customize messages for common errors
    switch (code) {
      case "file-too-large":
        const fileSizeInMB = (maxSize / 1024 / 1024).toFixed(2);
        if (files.length === 1) {
          messages.push(
            `${files[0]} exceeds the ${fileSizeInMB}MB size limit.`
          );
        } else {
          messages.push(
            `${files.length} files exceed the ${fileSizeInMB}MB size limit.`
          );
        }
        break;
      ...
      case "file-invalid-type":
        const acceptTypes =
          typeof accept === "object"
            ? Object.keys(accept)
                .map((type) =>
                  type.replace("application/", "").replace("image/", "")
                )
                .join(", ")
            : accept;
        ...
        if (files.length === 1) {
          messages.push(
            `${files[0]} has an unsupported file type. Accepted: ${acceptTypes}`
          );
        } else {
          messages.push(
            `${files.length} files have unsupported file types. Accepted: ${acceptTypes}`
          );
        }
        break;

      case "too-many-files":
        messages.push(`You can only upload up to ${maxFiles} files at once.`);
        break;

      default:
        // For other errors, use the default message
        if (files.length === 1) {
          messages.push(`${files[0]}: ${message}`);
        } else {
          messages.push(`${files.length} files: ${message}`);
        }
    }
  }
...
export default function DropZone({
  ...
  className,
  multiple = true,
  placeholder = "Drop or select images",
  maxFiles,
  setFiles,
}: Props) {
  ...
  const { getRootProps, getInputProps } = useDropzone({
    ...
    onDropAccepted: (files) => setFiles && setFiles(files),
    accept,
    disabled,
    maxSize,
    maxFiles,
  ...
  return (
    <>
      ...
      <label
        {...getRootProps({
          className: cn(
            "flex items-center justify-center select-noneP border py-6 rounded-lg border-border",
            disabled && "opacity-50 select-none",
            className
          ),
        })}
      >
        <DropZoneCard placeholder={placeholder} />
      </label>
    </>
  );
}
